class BookTranslationsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @translation.save
        format.html { redirect_to @book, notice: "Translation was successfully added." }
        format.turbo_stream {
          flash.now[:notice] = "Translation was successfully added."
          render turbo_stream: [
            turbo_stream.replace("book_translations", partial: "books/translations", locals: { book: @book }),
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to @book, notice: "Translation was successfully removed." }
      format.turbo_stream {
    ...
    respond_to do |format|
      if @translation.update(translation_params)
        ...
        format.turbo_stream {
          ...
          render turbo_stream: [
            turbo_stream.replace("book_#{@book.id}", partial: "books/book", locals: { book: @book }),
            turbo_stream.remove("modal"),
            turbo_stream.append("flash", partial: "shared/flash", locals: { flash: flash })
          ]
        }
