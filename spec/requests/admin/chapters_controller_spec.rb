...
RSpec.describe "Admin::Chapters", type: :request do
  ...
  describe "POST /admin/books/:book_id/chapters" do
    context "with valid parameters" do
      it "creates a new chapter and responds with Turbo Stream" do
        post admin_book_chapters_path(book), params: { chapter: { title: 'New Chapter', content: 'Content', position: 1 } }, headers: { 'Accept' => 'text/vnd.turbo-stream.html' }
    ...
    context "with invalid parameters" do
      it "does not create a new chapter and responds with Turbo Stream" do
        post admin_book_chapters_path(book), params: { chapter: { title: '', content: '', position: nil } }, headers: { 'Accept' => 'text/vnd.turbo-stream.html' }
  ...
  describe "PATCH /admin/books/:book_id/chapters/:id" do
    context "with valid parameters" do
      it "updates the chapter and responds with Turbo Stream" do
        patch admin_book_chapter_path(book, chapter.id), params: { chapter: { title: 'Updated Title' } }, headers: { 'Accept' => 'text/vnd.turbo-stream.html' }
    ...
    context "with invalid parameters" do
      it "does not update the chapter and responds with Turbo Stream" do
        patch admin_book_chapter_path(book, chapter.id), params: { chapter: { title: '' } }, headers: { 'Accept' => 'text/vnd.turbo-stream.html' }
  ...
  describe "DELETE /admin/books/:book_id/chapters/:id" do
    it "destroys the chapter" do
      ...
      }.to change(book.chapters, :count).by(-1)
